...
class Spot:
	...
	## Checking if the row that we are at is less than the total rows - 1 in order to
...
## Taking X & Y positon and dividing them by the width of the grid square
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN:
				if event.key == pygame.K_SPACE and start and end:
					...
					## Using lambda to pass draw fuction as an arguement to another function
